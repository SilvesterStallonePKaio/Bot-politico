...
def montar_contexto(noticias, label="NOTICIAS RECENTES"):
    ...
    contexto = f"{label}:\n" + "\n\n".join(linhas)
    # Prioriza links diretos (nao redirect Google News) para melhor preview
    links_diretos = [i for i in unicos if "news.google.com" not in i[2]]
    pool_links = links_diretos if links_diretos else unicos
    link1 = pool_links[0][2]
    link2 = pool_links[1][2] if len(pool_links) > 1 else link1
...
def parsear_rss(fonte, max_items=10, filtro_2h=False):
    ...
    try:
        ...
        for item in root.findall(".//item")[:max_items]:
            ...
            # Link do redirect Google News (gigante, mas fallback util)
            link_artigo = item.findtext("link", "").strip()
            # Prioriza homepage (gera preview no Telegram), fallback redirect
            link_final = link_homepage or link_artigo
...
def buscar_noticias_tema_completo(tema):
    ...
    # 1. NewsAPI — retorna links diretos das materias
    ...
    # 2. RSS Google News — bom para contexto, links nem sempre diretos
    rss_items = buscar_rss_por_tema(query, filtro_2h=True)
    print(f"[INFO] RSS Google: {len(rss_items)} noticias")

    # Monta contexto com tudo, mas coloca NewsAPI na frente
    # (montar_contexto vai priorizar links diretos para bloco 1 e 2)
    todos = newsapi_items + rss_items

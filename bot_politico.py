...
def montar_contexto(noticias, label="NOTICIAS RECENTES"):
    ...
    contexto = f"{label}:\n" + "\n\n".join(linhas)
    link1 = unicos[0][2]
    # Link alternativo: segunda noticia diferente (para Bloco 2)
    link2 = unicos[1][2] if len(unicos) > 1 and unicos[1][2] != link1 else link1
    veiculo1 = unicos[0][1] or "Google News"
...
def parsear_rss(fonte, max_items=10, filtro_2h=False):
    ...
    try:
        ...
        for item in root.findall(".//item")[:max_items]:
            ...
            # Link da materia especifica (redirect Google News)
            link_artigo = item.findtext("link", "").strip()
            # URL da homepage do veiculo (tag <source url="...">)
            source_el = item.find("source")
            link_homepage = source_el.get("url", "").strip() if source_el is not None else ""
            # Usa link_artigo como principal (mais especifico), homepage como fallback

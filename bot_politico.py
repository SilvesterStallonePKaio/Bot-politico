...
def montar_contexto(noticias, label="NOTICIAS RECENTES"):
    ...
    contexto = f"{label}:\n" + "\n\n".join(linhas)
    return contexto, unicos[0][2], unicos[0][1] or "Google News"

def resolver_link_google_news(url):
    """Resolve o link real de uma URL do Google News via decode base64."""
    if "news.google.com" not in url:
        return url
    try:
        import base64
        # Extrai o token do path /rss/articles/TOKEN
        ...
        if match:
            ...
            if padding != 4:
                token += '=' * padding
            decoded = base64.b64decode(token.replace('-', '+').replace('_', '/'))
            # Extrai URL do binario decodificado
            url_match = re.search(rb'https?://[^\x00-\x1f\x7f-\xff ]+', decoded)
            if url_match:
                return url_match.group(0).decode('utf-8', errors='ignore')
    except:
        pass
    # Fallback: parametro url=
...
def parsear_rss(fonte, max_items=10, filtro_2h=False):
    ...
    try:
        ...
        for item in root.findall(".//item")[:max_items]:
            ...
            link_raw = item.findtext("link", "").strip()
            # Resolve link real se for redirect do Google News
            link = resolver_link_google_news(link_raw) if link_raw else ""
            desc = limpar_html(item.findtext("description", ""))
            pub = item.findtext("pubDate", "")
            if titulo and len(titulo) > 15 and link:
                if filtro_2h and not dentro_de_2h(pub):
                    continue
                resultados.append((titulo, veiculo, link, desc, pub[:25]))
        return resultados
    except Exception as e:
...
def buscar_newsapi_por_tema(query, horas=2):
    ...
    try:
        ...
        for a in artigos:
            ...
            if titulo and len(titulo) > 15 and "[Removed]" not in titulo:
...
def buscar_noticias_tema_completo(tema):
    ...
    # 3. Se ainda pouco, tenta RSS sem filtro de tempo
    if len(todos) < 3:
        print(f"[INFO] Poucas noticias, expandindo busca RSS...")
        rss_extra = buscar_rss_por_tema(query, filtro_2h=False)
        todos.extend(rss_extra)
...
def buscar_noticias():
    """Busca automatica para posts agendados — prioriza ultimas 6h."""
    todos = []

    # 1. Google News RSS por temas quentes — filtro 6h
    temas_quentes = [
        "politica brasileira", "STF", "Lula governo",
        "congresso senado", "economia Brasil"
    ]
    tema = random.choice(temas_quentes)
    rss_recentes = buscar_rss_por_tema(tema, filtro_2h=False)
    # Filtra manualmente para 6h
    for item in rss_recentes:
        dt = parsear_data_rss(item[4]) if item[4] else None
        if dt:
            agora = datetime.now(dt.tzinfo) if dt.tzinfo else datetime.utcnow()
            if (agora - dt).total_seconds() <= 21600:  # 6h
            ...
            todos.append(item)
    print(f"[INFO] Agendado — RSS recente ({tema}): {len(todos)} noticias")

    # 2. FONTES_BREAKING com filtro 2h
    ...
    # 3. NewsAPI — ultimas 6h
    newsapi_items = buscar_newsapi_por_tema("politica Brasil", horas=6)
    todos.extend(newsapi_items)
    print(f"[INFO] Agendado — NewsAPI: {len(newsapi_items)} noticias")

    # 4. Fallback sem filtro se ainda pouco
    if len(todos) < 3:
        print(f"[INFO] Agendado — poucas noticias recentes, expandindo...")
...
def gerar_posts(tema_manual=None, bloco=1):
    ...
    if bloco == 2:
        ...
        contexto, link_fonte, veiculo = buscar_noticias_tema_completo(termo_busca)
        if contexto:
            pass  # ja formatado
        else:
        ...
        if is_url and eh_url_bloqueada(tema_manual):
            ...
            contexto, link_fonte, veiculo = buscar_noticias_tema_completo(tema_extraido)
            if not contexto:
            ...
            contexto, link_fonte, veiculo = buscar_noticias_tema_completo(tema_manual)
            if not contexto:
...
def buscar_e_confirmar(chat_id, tema):
    ...
    try:
        contexto, link, veiculo = buscar_noticias_tema_completo(tema)
...
def gerar_e_enviar(tema_manual=None, contexto_confirmado=None):
    try:
        if contexto_confirmado:
            ...
            # Bloco 2: busca complementar com NewsAPI
            enviar_telegram("⏳ _Gerando Bloco 2 (fontes complementares)..._")
            newsapi_items = buscar_newsapi_por_tema(tema, horas=6)
            if newsapi_items:
                ctx2, lnk2, vec2 = montar_contexto(newsapi_items, label="COBERTURA COMPLEMENTAR")
            else:
                ctx2, lnk2, vec2 = contexto, link, veiculo
            posts2, _ = gerar_posts_com_contexto(ctx2 or contexto, lnk2 or link, vec2 or veiculo)
            ...
            contexto, link, veiculo = buscar_noticias()
            if contexto:

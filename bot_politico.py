...
FONTES_BREAKING = [
    # Breaking news / hard news — mais recentes primeiro
    "https://news.google.com/rss/topics/CAAqJggKIiBDQkFTRWdvSUwyMHZNRFZxYVdjU0FtcHZHZ0pTVWlnQVAB?hl=pt-BR&gl=BR&ceid=BR:pt-419",  # Top stories Brasil
...
FONTES_POLITICA = [
    # Politica especifica — temas quentes
...
# Veiculos confiaveis para filtrar resultados
VEICULOS_CONFIAVEIS = [
...
def parsear_rss(fonte, max_items=8):
    """Parseia um feed RSS e retorna lista de (titulo, veiculo, link, desc, data)."""
...
def buscar_noticias():
    """Busca hard news / breaking news politicas. Retorna (texto, link, veiculo)."""
    todos = []

    # 1. Tenta breaking news primeiro
    ...
    # 2. Adiciona fontes de politica especifica
    fontes_pol = FONTES_POLITICA.copy()
    random.shuffle(fontes_pol)
    for fonte in fontes_pol[:4]:  # tenta 4 fontes politicas
        try:
            items = parsear_rss(fonte)
            todos.extend(items)
        except Exception as e:
    ...
    # Pega a noticia principal (primeira confiavel)
    principal = unicos[0]
    titulo_p, veiculo_p, link_p, desc_p, data_p = principal

    # Monta contexto com as top noticias
    contexto_items = []
...
def buscar_conteudo_url(url):
    """Busca conteudo de uma URL especifica para usar como fonte."""
    try:
        ...
        # Pega dominio como nome do veiculo
...
VERSAO5: [texto completo]"""
    ...
    if resp.status_code != 200:
        raise Exception(f"Gemini erro {resp.status_code}: {resp.text[:200]}")
    
    resposta = resp.json()["candidates"][0]["content"]["parts"][0]["text"].strip()
    print(f"[INFO] Resposta Gemini:\n{resposta[:300]}")
...
def main():
    ...
    for var in ["GEMINI_API_KEY", "TELEGRAM_BOT_TOKEN", "TELEGRAM_CHAT_ID", "YOUTUBE_API_KEY"]:

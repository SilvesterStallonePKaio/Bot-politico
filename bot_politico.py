...
def gerar_e_enviar(tema_manual=None, contexto_confirmado=None):
    try:
        if contexto_confirmado:
            ...
            contexto2, link2, veiculo2 = buscar_newsapi_por_tema(tema, horas=6), link, veiculo
            # monta contexto2 se NewsAPI retornou algo diferente
            if contexto2 and isinstance(contexto2, list) and len(contexto2) > 0:
                ctx2, lnk2, vec2 = montar_contexto(contexto2, label="COBERTURA COMPLEMENTAR")
                if ctx2:
                    posts2, _ = gerar_posts_com_contexto(ctx2, lnk2, vec2)
                else:
                    posts2, _ = gerar_posts_com_contexto(contexto, link, veiculo)
                    vec2 = veiculo
            else:
                posts2, _ = gerar_posts_com_contexto(contexto, link, veiculo)
                vec2 = veiculo
            enviar_bloco(posts2, vec2, "🔍 *@OlhaQueSurpresa* — Bloco 2")

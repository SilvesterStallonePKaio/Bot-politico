...
def parsear_rss(fonte, max_items=10, filtro_2h=False):
    ...
    try:
        ...
        for item in root.findall(".//item")[:max_items]:
            ...
            # Tenta extrair link real do atributo url da tag <source>
            source_el = item.find("source")
            link_real = source_el.get("url", "").strip() if source_el is not None else ""
            # Fallback: usa o <link> do item (pode ser redirect Google)
            if not link_real:
                link_real = item.findtext("link", "").strip()
            desc = limpar_html(item.findtext("description", ""))
            pub = item.findtext("pubDate", "")
            if titulo and len(titulo) > 15 and link_real:
                if filtro_2h and not dentro_de_2h(pub):
                    continue
                resultados.append((titulo, veiculo, link_real, desc, pub[:25]))
        return resultados
    except Exception as e:

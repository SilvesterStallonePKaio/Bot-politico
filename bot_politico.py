...
def extrair_palavras_chave(texto):
    """Extrai palavras-chave relevantes de um texto/URL para busca RSS."""
    # Remove URL scheme e dominio se for URL
    texto = re.sub(r'https?://[^/]+/', ' ', texto)
    # Troca hifens e underscores por espacos
    texto = re.sub(r'[-_]', ' ', texto)
    # Remove palavras comuns/stopwords
    stopwords = {'de', 'da', 'do', 'das', 'dos', 'e', 'o', 'a', 'os', 'as', 'em',
                 'no', 'na', 'nos', 'nas', 'por', 'para', 'com', 'que', 'se', 'um',
                 'uma', 'ao', 'aos', 'html', 'php', 'com', 'br', 'www', 'noticia',
                 'politica', 'noticias', 'brasil', 'sobre', 'mais', 'este', 'essa'}
    palavras = [p for p in re.findall(r'[a-zA-ZÀ-ú]{4,}', texto.lower()) if p not in stopwords]
    # Retorna as 4 palavras mais longas (tendem a ser mais especificas)
    palavras_unicas = list(dict.fromkeys(palavras))
    palavras_unicas.sort(key=len, reverse=True)
    return palavras_unicas[:4]

def buscar_noticias_por_tema(tema):
    """Busca noticias recentes no Google News RSS sobre um tema especifico."""
    palavras = extrair_palavras_chave(tema)
    if not palavras:
        return None, None, None

    query = '+'.join(palavras)
    url = f"https://news.google.com/rss/search?q={query}&hl=pt-BR&gl=BR&ceid=BR:pt-419&sort=date"
    print(f"[INFO] Buscando RSS tema: {query}")

    try:
        items = parsear_rss(url, max_items=8)
        if not items:
            return None, None, None
        ...
        contexto = "\n\n".join(contexto_items)
        link_principal = unicos[0][2]
        veiculo_principal = unicos[0][1] or "Google News"
        print(f"[INFO] RSS tema: {len(unicos)} noticias encontradas")
        return contexto, link_principal, veiculo_principal
    except Exception as e:
        print(f"[AVISO RSS tema] {e}")
        return None, None, None

def eh_url_bloqueada(url):
    """Detecta URLs que bloqueiam scrapers (X, Twitter, Instagram, etc)."""
...
def extrair_tema_da_url(url):
    """Extrai tema legivel de uma URL para usar na busca RSS."""
    from urllib.parse import urlparse
    parsed = urlparse(url)
    dominio = parsed.netloc.replace("www.", "").split(".")[0]
    path = parsed.path.replace("/", " ").replace("-", " ").replace("_", " ")
    # Remove numeros longos (IDs de status do X/Twitter)
...
def gerar_posts(tema_manual=None, bloco=1):
    ...
    if bloco == 2:
        # Nova logica: busca RSS por palavras-chave do tema
        termo_busca = tema_manual or "politica brasil escandalo"
        contexto, link_fonte, veiculo = buscar_noticias_por_tema(termo_busca)
        if contexto:
            contexto = f"NOTICIAS RECENTES SOBRE O TEMA (Google News):\n{contexto}"
        else:
        ...
        if is_url and eh_url_bloqueada(tema_manual):
            # URL bloqueada (X, Twitter, etc): extrai tema e busca no RSS
            print(f"[INFO] URL bloqueada detectada, usando RSS: {tema_manual}")
            tema_extraido = extrair_tema_da_url(tema_manual)
            contexto, link_fonte, veiculo = buscar_noticias_por_tema(tema_extraido)
            if contexto:
                contexto = f"NOTICIAS RECENTES SOBRE O TEMA (Google News):\n{contexto}"
                print(f"[INFO] RSS encontrou contexto para: {tema_extraido}")
            else:
                contexto = f"URL do X/Twitter enviada: {tema_manual}\nBusca RSS nao retornou resultados. Use o briefing."
        elif is_url:
            # URL normal: tenta raspar, se falhar usa RSS
            ...
            if conteudo:
                ...
                # Fallback: tenta RSS com palavras da URL
                print(f"[INFO] Raspagem falhou, tentando RSS como fallback")
                tema_extraido = extrair_tema_da_url(tema_manual)
                contexto, link_fonte, veiculo = buscar_noticias_por_tema(tema_extraido)
                if contexto:
                    contexto = f"NOTICIAS RECENTES SOBRE O TEMA (Google News):\n{contexto}"
            ...
            # Texto/tema livre: usa diretamente + busca RSS
            contexto_rss, link_fonte, veiculo = buscar_noticias_por_tema(tema_manual)
            if contexto_rss:
...
def enviar_bloco(posts, veiculo, titulo_bloco):
    """Envia um bloco de posts formatados no Telegram."""
    fonte_info = f"📡 _{veiculo}_" if veiculo else "📚 _Contexto geral_"
    enviar_telegram(f"{titulo_bloco}\n{fonte_info}")
    time.sleep(0.5)
...
def gerar_e_enviar(tema_manual=None):
    try:
        ...
        if tema_manual:
            # BLOCO 1: logica original (URL raspada ou tema direto)
            enviar_telegram("⏳ _Gerando Bloco 1 (materia original)..._")
            posts1, veiculo1 = gerar_posts(tema_manual, bloco=1)
            enviar_bloco(posts1, veiculo1, "🇧🇷 *@OlhaQueSurpresa* — Bloco 1: Materia original")
            print(f"[INFO] Bloco 1: {len(posts1)} posts")

            time.sleep(1)

            # BLOCO 2: logica nova (RSS Google News por palavras-chave)
            enviar_telegram("⏳ _Gerando Bloco 2 (noticias recentes do tema)..._")
            posts2, veiculo2 = gerar_posts(tema_manual, bloco=2)
            enviar_bloco(posts2, veiculo2, "🔍 *@OlhaQueSurpresa* — Bloco 2: Contexto Google News")
            print(f"[INFO] Bloco 2: {len(posts2)} posts")

            enviar_telegram("_Escolha uma opcao acima ou ignore. Total: 10 opcoes._")
        else:
            # Posts automaticos agendados: logica original, 5 opcoes
